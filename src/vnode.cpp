...
bool CVnodeBroadcast::CheckOutpoint(int &nDos) {
...

    {
        ...
        if (coins.vout[vin.prevout.n].nValue != VNODE_COIN_REQUIRED * COIN) {
            LogPrint("vnode", "CVnodeBroadcast::CheckOutpoint -- Vnode UTXO should have 3750 VTL, vnode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 3750 VTL tx got nVnodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex *pMNIndex = (*mi).second; // block for 3750 VTL tx -> 1 confirmation
