...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector <CompactTallyItem> &vecTallyRet, bool fSkipDenominated,
    ...
    vecTallyRet.clear();
    for (const PAIRTYPE(CBitcoinAddress, CompactTallyItem)&item : mapTally) {
...
//                bool fFirst = true;
                ...
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl)) {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 3750 VTL.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 3750 VTL.");
...
//                bool fAllowFree = false;					// No free TXs in VTL
...
//            LogPrintf("txNew:%s\n", txNew.ToString());
            ...
            for (const CZerocoinEntry &minIdPubcoin : listPubCoin) {
        ...
        CWalletDB(strWalletFile).WriteZerocoinEntry(zerocoinSelected);*/
            ...
            for (const CZerocoinSpendEntry &item : listCoinSpendSerial){
